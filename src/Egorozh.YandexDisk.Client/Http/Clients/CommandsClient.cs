...
internal class CommandsClient(ApiContext apiContext) : DiskClientBase(apiContext), ICommandsClient
{
    ...
    {
        string query = string.IsNullOrWhiteSpace(request.Name)
            ? GetQuery("path", request.Path, "overwrite", request.Overwrite.ToString().ToLower())
            : GetQuery("path", request.Path, "overwrite", request.Overwrite.ToString().ToLower(), "name", request.Name);
